...
# FloodGuard Penang — Stations Page

**Route:** `/stations`

## 1. Purpose

Find, filter and compare all JPS rainfall and water-level stations in Penang.

## 2. Header

- Title: `Stations`
- Subtitle: `All JPS rainfall and water-level stations in Penang`
- Last updated
- Refresh
...
Selected segment uses blue subtle background.

## 4. Main table — approximately 9 columns
...
- Type
- Latest rainfall (mm)
- Latest water level (m)
- Last observation (MYT)
- Freshness
- Condition
- Action
...
- status chips
- `View` button
- pagination / row-count footer when required

## 5. Right insight rail — approximately 3 columns

### Stations by type

Compact distribution visualization with:
...
Horizontal bars for district station counts.
...
Rainfall-only stations must not be assigned official JPS water-level severity.

## 7. Empty/error states

- no station match → compact empty state + clear filters
- API error → in-table error + Retry
