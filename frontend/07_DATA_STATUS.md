...
# FloodGuard Penang — Data Status Page

**Route:** `/status`

## 1. Purpose

Show whether FloodGuard's upstream data, services and prediction pipeline are trustworthy and operational.

## 2. Header

- `Data status`
- subtitle: `Source availability and system health for real-time data and model services`
- Last updated
- Refresh
- district selector only if useful to station-level quality summaries

## 3. Top row

### Data source status — approximately 5 columns

Compact source cards:

- JPS Rainfall
- JPS Water-level
- METMalaysia Weather, only if actually integrated
...
### Service health — approximately 4 columns

Compact service cells:
...
- latency or heartbeat
- last run where relevant

### Latest successful retrieval — approximately 3 columns

List latest retrieval for each supported source/service.

## 4. Middle row

### Station freshness — approximately 4 columns
...
A compact distribution graphic is acceptable for this design style.

Include explanatory note for freshness rules.

### Data quality issues — approximately 8 columns
...
- Last successful run
- Next run
- Uptime / heartbeat if available

## 6. Sources and disclaimers

Bottom full-width panel with four concise source blocks:

1. JPS observed data
2. FloodGuard freshness labels
3. FloodGuard predictions
4. Not an official warning service
