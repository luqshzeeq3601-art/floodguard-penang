"""Identify provisional flood/threshold events in ONE processed dataset, in one offline run.

Reads processed/observations/v1/<dataset_version>/ (directory or its dataset_manifest.json),
verifies the Phase 2 contract (manifest hash, checks, schemas), segments contiguous threshold
exceedance episodes against current reference thresholds (Waspada, Amaran, Bahaya; NORMAL
excluded) and writes <output-root>/<dataset_version>/threshold_events.json
(write-once; rerun is a no-op). No network access. Design: docs/EVENT_IDENTIFICATION.md.

Outputs derived from JPS data are PERMISSION REQUIRED and stay local (git-ignored data/analysis/).

Usage (repo root):
    .venv\\Scripts\\python.exe scripts\\identify_events.py --dataset DIR
...
from floodguard.analysis.events import analyze, write_summary
...
def main(argv: Sequence[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Identify provisional threshold events (offline).")
    ap.add_argument("--dataset", type=Path, required=True, help="processed dataset dir/manifest")
    ap.add_argument("--output-root", type=Path, default=ROOT / "data" / "analysis" / "events")
    ap.add_argument(
        "--station-master-dir",
        type=Path,
        ...
        json.dumps(
            {
                ...
                "usable_observations": doc["network"]["usable_observations"],
                "total_episodes_by_threshold_type": doc["network"][
                    "total_episodes_by_threshold_type"
                ],
