"""Define +30/+60/+120 min prediction labels for ONE processed dataset, in one offline run.

Reads processed/observations/v1/<dataset_version>/ (directory or its dataset_manifest.json),
verifies the Phase 2 contract (manifest hash, checks, schemas), evaluates future water levels
and proxy threshold exceedances (+30, +60, +120 min) and writes
<output-root>/<dataset_version>/labels.json (write-once; rerun is a no-op).
No network access. Design: docs/LABEL_DEFINITION.md.

Outputs derived from JPS data are PERMISSION REQUIRED and stay local (git-ignored data/analysis/).

Usage (repo root):
    .venv\\Scripts\\python.exe scripts\\define_labels.py --dataset DIR
...
from floodguard.analysis.labels import analyze, write_summary
...
def main(argv: Sequence[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Define +30/+60/+120 prediction labels (offline).")
    ap.add_argument("--dataset", type=Path, required=True, help="processed dataset dir/manifest")
    ap.add_argument("--output-root", type=Path, default=ROOT / "data" / "analysis" / "labels")
    ap.add_argument(
        "--station-master-dir",
        type=Path,
